...
describe('access token middleware', async function() {
  ...
  it('should generate an access token', async function() {
    try {
      ...
      const id = mongoose.Types.ObjectId().toString();
      const email = 'jd@example.net';
      const accessToken = await authMiddleware.generateAccessToken({
        '_id': id,
        'email': email
      });
      ...
      const { payload, protectedHeader } = await jose.jwtDecrypt(accessToken, key, { 'issuer': 'flyquackswim.com/al-btn' });
      expect(payload).to.be.a('object');
      expect(payload.userId).to.be.equal(id);
      expect(payload.email).to.be.equal(email);
      expect(payload.iss).to.be.equal('flyquackswim.com/al-btn');
